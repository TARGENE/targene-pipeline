...
- `PREVALENCE_FILE` : An optional tab-separated file mapping binary trait to their expected prevalence.
- `PREVALENCE_MODE (default: sampling)`: Defines the method by which outcome prevalence correction will be performed. Outcome specific target prevalences are specified in the `PREVALENCE_FILE`. If set to "sampling", cases or controls will be downsampled to reach the target prevalence. If set to "ccw", the semi-parametric estimator will be modified to account for the target prevalence.
