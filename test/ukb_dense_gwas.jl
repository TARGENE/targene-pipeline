...
@testset "Test ukb_dense_gwas.config" begin
    cmd = `nextflow run main.nf -c test/configs/ukb_dense_gwas.config $args`
    ...
    for chr in 1:3
        dataset_file = joinpath("results", "datasets", "dense.ukb_chr$(chr).data.arrow")
        ...
        # Dense mode extracts variants within WINDOW_SIZE of prioritized variants
    ...
    @test isfile(joinpath("results","pve","pve.dense.ukb_chr1.txt"))
    @test isfile(joinpath("results","pve","pve.dense.ukb_chr2.txt"))
    @test isfile(joinpath("results","pve","pve.dense.ukb_chr3.txt"))
